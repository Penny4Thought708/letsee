...
export class WebRTCController {
  ...
  async handleOffer(data) {
    const { from, offer, fromName, audioOnly } = data || {};
    if (!from || !offer) {
      console.warn("[WebRTC] handleOffer: invalid data", data);
      return;
    }

    const displayName = fromName || `User ${from}`;
  ...
  /* ---------------------------------------------------
     Remote End (local-only cleanup hook if you add one later)
  --------------------------------------------------- */
  handleRemoteEnd() {
    this.endCall(false);
  }
  ...
  endCall(local = true) {
    ...
    // No server-side "end" event in your old protocol; this is local-only
    this.onCallEnded?.();
  }
